...
void io_display(dungeon *d)
{
  ...
  * not otherwise used.                                                 */
  ...
  mvprintw(23, 1, "%s position is (%2d,%2d), current health %d , current level %d", d->PC->name, 
           d->PC->position[dim_x], d->PC->position[dim_y], d->PC->hp, d->PC->level);
...
{
  clear();
  mvprintw(10, 33, " Character Stats");
  mvprintw(11, 33, " HP:    %5d ", d->PC->hp);
  mvprintw(12, 33, " Speed: %5d ", d->PC->speed);
  mvprintw(13, 33, " Gold: %5d ", d->PC->gold);
  mvprintw(14, 33, " Experience: %5d ", d->PC->experience);
  mvprintw(15, 33, " Level: %5d ", d->PC->level);
...
uint32_t io_wear_eq(dungeon *d)
{
  ...
  char s[61];
  clear();
  mvprintw(5, 10 , "Player Inventory \U0001F41D :");
...
uint32_t io_display_shop(dungeon *d){
  uint32_t i, j, current;
  int key;
  char s[61];
  clear();
  mvprintw(2, 10, "Shop: Current Gold: %d", d->PC->gold);
  current = 0;
  for(i = 0; i  < 10; i++){
    if(d->shop[i] !=NULL){
      mvprintw(i+3, 10, "%d : %s,          Cost: %d", i,  d->shop[i]->get_name(), d->shopCosts[i]);
    }    
  }
  mvprintw(20, 10, "Select 0 - 9 to buy, down and up arrow keys to scroll");    
  while (1) {
    if ((key = getch()) == 27 /* ESC */) {
      io_display(d);
      return 1;
    ...
    if (key < '0' || key > '9' || key == KEY_DOWN || key == KEY_UP) {
      if (isprint(key)) {
        snprintf(s, 61, "Invalid input: '%c'.  Enter 0-9 or ESC to cancel.",
                 key);
        mvprintw(18, 10, " %-58s ", s);
    ...
    if(key == '0' && d->PC->gold > d->shopCosts[current]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current];
          d->shop[current] = NULL;          
          d->PC->gold -=  d->shopCosts[current];
          d->shopCosts[current] = 0;
          break;
        }
      }
      j = 0;
      for(i = current; i < current + 10; i++){
        if(d->shop[i] == NULL){
          ...
          mvprintw(j+3, 10, "%d : %s,          Cost: %d  ", j,  d->shop[i]->get_name(), d->shopCosts[i]);
        }
        j++;        
      }      
      mvprintw(2, 10, "Shop: Current Gold: %d  ", d->PC->gold);
    ...
    if(key == '1' && d->PC->gold > d->shopCosts[current + 1]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 1];
          d->shop[current + 1] = NULL;          
          d->PC->gold -= d->shopCosts[current + 1];
          d->shopCosts[current + 1] = 0;
          break;
        }
      }
      j = 0;
    ...
    if(key == '2' && d->PC->gold > d->shopCosts[current + 2]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 2];
          d->shop[current + 2] = NULL;          
          d->PC->gold -= d->shopCosts[current + 2];
          d->shopCosts[current + 2] = 0;
          break;
        }
      }
      j = 0;
    ...
    if(key == '3' && d->PC->gold > d->shopCosts[current + 3]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 3];
          d->shop[current + 3] = NULL;          
          d->PC->gold -= d->shopCosts[current + 3];
          d->shopCosts[current + 3] = 0;
          break;
        }
      }
      j = 0;
    ...
    if(key == '4' && d->PC->gold > d->shopCosts[current + 4]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 4];
          d->shop[current + 4] = NULL;          
          d->PC->gold -= d->shopCosts[current + 4];
          d->shopCosts[current + 4] = 0;
          break;
        }
      }
      j = 0;
    ...
    if(key == '5' && d->PC->gold > d->shopCosts[current + 5]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 5];
          d->shop[current + 5] = NULL;          
          d->PC->gold -= d->shopCosts[current + 5];
          d->shopCosts[current + 5] = 0;
          break;
        }
      }
      j = 0;
    ...
    if(key == '6' && d->PC->gold > d->shopCosts[current + 6]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 6];
          d->shop[current + 6] = NULL;          
          d->PC->gold -= d->shopCosts[current + 6];
          d->shopCosts[current + 6] = 0;
          break;
        }
      }
      j = 0;
    ...
    if(key == '7' && d->PC->gold > d->shopCosts[current + 7]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 7];
          d->shop[current + 7] = NULL;          
          d->PC->gold -= d->shopCosts[current + 7];
          d->shopCosts[current + 7] = 0;
          break;
        }
      }
      j = 0;
    ...
    if(key == '8' && d->PC->gold > d->shopCosts[current + 8]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 8];
          d->shop[current + 8] = NULL;          
          d->PC->gold -= d->shopCosts[current + 8];
          d->shopCosts[current + 8] = 0;
          break;
        }
      }
      j = 0;
    ...
    if(key == '9' && d->PC->gold > d->shopCosts[current + 9]){
      for(i = 0; i < 10; i++){
        if(d->PC->in[i] == NULL){
          d->PC->in[i] = d->shop[current + 9];
          d->shop[current + 9] = NULL;          
          d->PC->gold -= d->shopCosts[current + 9];
          d->shopCosts[current + 9] = 0;
          break;
        }
      }
      j = 0;
...
uint32_t io_level_up(dungeon *d)
{
  uint32_t key;
  char s[61];
  clear();
  mvprintw(5, 10, "Player Attributes: ");
  // for (i = 0; i < MAX_INVENTORY; i++) {
  //   /* We'll write 12 lines, 10 of inventory, 1 blank, and 1 prompt. *
  //    * We'll limit width to 60 characters, so very long object names *
  //    * will be truncated.  In an 80x24 terminal, this gives offsets  *
  ...
  mvprintw(6, 10, " [0] Strenght: %d", d->PC->strength);
  mvprintw(7, 10, " [1] Stamina: %d", d->PC->stamina);
  mvprintw(8, 10, " [2] Dexterity: %d", d->PC->dexterity);


  mvprintw(16, 10, " %-58s ", "");
  mvprintw(18, 10, " %-58s ", "Level up what attribute (ESC to cancel)?");
  mvprintw(17, 10, "You have %d attributes points: ", d->PC->attribute_points);
  ...
  while (1) {
    ...
    if (key < '0' || key > '2') {
      if (isprint(key)) {
        snprintf(s, 61, "Invalid input: '%c'.  Enter 0-2 or ESC to cancel.",
                 key);
        mvprintw(18, 10, " %-58s ", s);
      } else {
        mvprintw(18, 10, " %-58s ",
                 "Invalid input.  Enter 0-2 or ESC to cancel.");
      }
      refresh();
      continue;
    }
    if(d->PC->attribute_points == 0){
      mvprintw(21, 10, " %-58s ",
                 "Not enough attribute points.  Enter 0-9 or ESC to cancel.");
      refresh();
    }
    if(key == '0' && d->PC->attribute_points > 0){
      d->PC->strength++;
      d->PC->attribute_points--;
      mvprintw(19, 10, "Strenght was increased by 1.");
      refresh();
    }
    if(key == '1' && d->PC->attribute_points > 0){
      d->PC->stamina++;
      d->PC->attribute_points--;
    }
    if(key == '2' && d->PC->attribute_points > 0){
      d->PC->dexterity++;
      d->PC->attribute_points--;
    }
    // if (!d->PC->in[key - '0']) {
...
static uint32_t io_inspect_in(dungeon *d)
{
  ...
  char s[61];
  clear();
  mvprintw(5, 10, "Player Inventory: ");
  for (i = 0; i < MAX_INVENTORY; i++) {
    io_object_to_string(d->PC->in[i], s, 61);
    mvprintw(i + 6, 10, " %c) %-55s ", '0' + i,
             d->PC->in[i] ? d->PC->in[i]->get_name() : "");
