...
void do_combat(dungeon *d, character *atk, character *def)
{
  ...
  if (character_is_alive(def)) {
    if (atk != d->PC) {
      damage = atk->damage->roll() + (d->floor_level * 10);
      defence = atk->defence; 
      ...
      for (i = damage = 0; i < num_eq_slots; i++) {
        if (i == eq_slot_weapon && !d->PC->eq[i]) {
          ...
          damage = damage + (d->PC->strength * 5); 
          defence = 0;
        } else if (d->PC->eq[i]) {
          damage += d->PC->eq[i]->roll_dice();
          damage = damage + (d->PC->strength * 5);
          d->PC->speed += ((d->PC->eq[i]->get_speed())) * (d->PC->dexterity * 2);
          defence += (d->PC->eq[i]->get_defence() + (d->PC->stamina * 3)); 
        }
      }
      io_queue_message("You hit %s%s for %d. It has %d health left", is_unique(def) ? "" : "the ",
    ...
    if (damage >= def->hp) {
      if (atk != d->PC) {
        ...
        d->PC->gold += def->gold;
        d->PC->experience += def->experience;
        calculate_level(d); 
      }
      def->hp = 0;
      def->alive = 0;
